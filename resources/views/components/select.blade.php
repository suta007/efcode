...
<div class="mb-3 grid content-center gap-x-4 md:grid-cols-6">
	...
	<select {{ $required ? 'required' : '' }} name="{{ $val }}" id="{{ $val }}" {!! $attributes->merge(['class' => 'rounded border border-gray-300 bg-gray-50 py-1 px-2 focus:border-web-900 focus:ring-web-900']) !!}>
		{{ $slot }}
	</select>
	@if ($errors->has($val))
