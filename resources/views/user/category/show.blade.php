...
@section('content')
	<x-div class="mx-auto fw-bold fs-5 mb-2 col-12">ข้อมูลCategory</x-div>
	<div class="clearfix"></div>
	<x-div class="mx-auto px-5 col-12">
		<a href="{{ route('user.category.edit', $data->id) }}" data-tooltip="แก้ไข{{ $data->name }}" class="btn btn-primary btn-sm float-start me-2"><i class="fa-solid fa-pen-to-square me-2"></i> แก้ไข</a>
		<form method="POST" action="{{ route('user.category.destroy', $data->id) }}" accept-charset="UTF-8" style="display:inline">
			@method('DELETE')
			@csrf
